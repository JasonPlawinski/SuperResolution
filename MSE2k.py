...
def BuildTestImage(LR, Generated, Target, epoch):
    ...
    if epoch < 1000:
        ZerosIncrement += '0'
    name = "./Results/MSE2k/Epoch" + ZerosIncrement + str(epoch) + ".png"
    TestImage.save(name)
...
def ComputeEdges(HRtarget):
    target = []
    b = HRtarget.numpy()
    for i in range(len(b)):
        gray = rgb2gray(b[i])
        edge = np.array(feature.canny(gray), dtype = np.float).reshape([1,256,256])
        target.append(edge)
...
def main(opt):
    ...
    scheduler_Up = lr_scheduler.StepLR(optimizer_Up, 500, gamma=0.1)

    print("===> Training")
    EpochLossList = []
    for epoch in range(0, opt.nEpochs + 1):
        a = trainNetwork(training_data_loader, testing_data_loader, optimizer_Up, model, criterionMSE, 
                     criterionL1, epoch, opt.batchSize, EpochLossList)
        if a == 0:
            break
        scheduler_Up.step()
    
    np.savetxt('./Results/MSE2k.txt', np.array(EpochLossList))
    
    
def trainNetwork(training_data_loader, testing_data_loader, optimizer_Up, model, criterionMSE, criterionL1, epoch, batch_size, EpochLossList):
    print ("epoch =", epoch)
    model.train()
    LossList = []
        ...
        if iteration%800 == 0:
            ...
            if epoch%1 == 0:
                ...
                BuildTestImage(LRtest[0,:,:,:].data.cpu(), HRTest[0,:,:,:].data.cpu(), 
                               GTtest[0,:,:,:], epoch)
            if epoch%500 == 0:
                SRGname = "./NetworkSaves/MSE2k" + str(epoch) + ".pt"
