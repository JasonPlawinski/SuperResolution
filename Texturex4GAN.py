...
def BuildTestImage(LR, Generated, Target, epoch):
    ...
    if epoch < 1000:
        ZerosIncrement += '0'
    name = "./Results/TextureMSEx4GAN/Epoch" + ZerosIncrement + str(epoch) + ".png"
    TestImage.save(name)
...
def main(opt):
    ...
    scheduler_Up = lr_scheduler.StepLR(optimizer_Up, 200, gamma=0.1)
    scheduler_D = lr_scheduler.StepLR(optimizer_Up, 200, gamma=0.1)
    ...
    np.savetxt('./Results/TextureMSEx4GAN/Loss_GAN.txt', np.array(EpochLossList))
    np.savetxt('./Results/TextureMSEx4GAN/G_GAN.txt', np.array(G_LossList))
    np.savetxt('./Results/TextureMSEx4GAN/D_GAN.txt', np.array(D_LossList))
    np.savetxt('./Results/TextureMSEx4GAN/MSE_GAN.txt', np.array(MSE_LossList))
...
def trainNetwork(training_data_loader, testing_data_loader, optimizer_Up, optimizer_D, UPmodel, D, Sobel, criterionMSE, criterionBCE, criterionL1, epoch, batch_size, EpochLossList, G_LossList, D_LossList, MSE_LossList):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        g_loss = MSELoss + 5*1e-3 * g_loss_GAN
        ...
        if iteration%2000 == 0:
            ...
            if epoch%1 == 0:
                ...
                BuildTestImage(LRtest[0,:,:,:].data.cpu(), HRTest[0,:,:,:].data.cpu(), 
                               GTtest[0,:,:,:], epoch)
            if epoch%400 == 0:
                SRGname = "./NetworkSaves/GAN_MSE_4x_" + str(epoch) + ".pt"
