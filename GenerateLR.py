...
def BuildTestImage(LR, epoch):
    ...
    TestImage = CompareImageDelimit(upLR)
    ...
    if epoch < 1000:
        ZerosIncrement += '0'
    name = "./NetworkEvaluation/LR/" + ZerosIncrement + str(epoch) + ".png"
    TestImage.save(name)
...
def main(opt):
    ...
    print("===> Model load")
    pretrained_dict = torch.load("NetworkSaves/FFT2k_0.5_1000.pt")
    ...
    for iteration, batch in enumerate(testing_data_loader, 1):
        print(iteration)
        batchM = batch[0][:,:,:,:]
        LRinput = Variable(batchM[:,:,::4,::4], requires_grad=False).cuda()
