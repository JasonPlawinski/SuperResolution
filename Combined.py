...
def BuildTestImage(LR, Generated, Target, epoch):
    ...
    if epoch < 1000:
        ZerosIncrement += '0'
    name = "./Results/Combine/Epoch" + ZerosIncrement + str(epoch) + ".png"
    TestImage.save(name)
...
def main(opt):
    ...
    np.savetxt('./Results/Combine.txt', np.array(EpochLossList))
...
def trainNetwork(training_data_loader, testing_data_loader, optimizer_Up, model, Sobel, criterionMSE, criterionL1, epoch, batch_size, EpochLossList):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        Loss = MSELoss + alphaS*SLoss + alphaFFT*FFTLossL1
        ...
        SLossList.append(alphaS*SLoss.data[0])
        FFTLossList.append(alphaFFT*FFTLossL1.data[0])
        
        #Plotting and following the progress
        if iteration%4000 == 0:
            print("===> Epoch[{}]({}/{}) MSE {:.5f}, SLoss {:.5f}, FFTLoss {:.5f}, Loss {:.5f}".format(epoch, iteration, len(training_data_loader), MSELoss.data[0], alphaS*SLoss.data[0], alphaFFT*FFTLossL1.data[0], Loss.data[0]))
            if epoch%1 == 0:
                model.eval()
                ...
                BuildTestImage(LRtest[0,:,:,:].data.cpu(), HRTest[0,:,:,:].data.cpu(), 
                               GTtest[0,:,:,:], epoch)
            if epoch%250 == 0:
                SRGname = "./NetworkSaves/Combine_" + str(epoch) + ".pt"
    ...
    FFTLossMean = np.mean(np.array(FFTLossList))
    EpochLossList.append(LossMean)
    print("===> MSE {:.5f}, SLoss {:.5f}, FFTLoss {:.5f}, Loss {:.5f}".format(MSELossMean, SLossMean, FFTLossMean, LossMean))
